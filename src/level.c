...
static void draw_rect(Level *level, int x1, int y1, int x2, int y2, TileType tile) {
    for (int y = y1; y <= y2; y++) {
        for (int x = x1; x <= x2; x++) {
...
void level_init(Level *level) {
    ...
    // Staircase
    for (int i = 0; i < 8; i++) {
        draw_rect(level, 160 + i, LEVEL_HEIGHT - 3 - i, 160 + i, LEVEL_HEIGHT - 3, TILE_BRICK);
    }

    // Down staircase
    for (int i = 0; i < 8; i++) {
        draw_rect(level, 168 + i, LEVEL_HEIGHT - 11 + i, 168 + i, LEVEL_HEIGHT - 3, TILE_BRICK);
    }
