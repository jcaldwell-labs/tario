...
#define JUMP_CUT_MULTIPLIER 0.5f  // Multiply velocity by this when jump released early
...
void player_update(Player *player, float delta_time) {
    ...
    // Keep player in horizontal bounds (will be removed when camera scrolling is added)
...
void player_jump_press(Player *player) {
    ...
    // This check will also be called from game logic when landing (for jump buffering)
